...
function mockRole(role: string) { return { _tag: 'Role', value: role }; }
function mockAriaLabel(label: string) { return { _tag: 'AriaLabel', value: label }; }
function mockAttr(name: string, value: string) { return { name, value }; }
...
describe('collectAttr', () => {
  ...
  it('finds aria-label via h.Attribute pattern', () => {
    const node = mockEl('g', [mockAttr('aria-label', 'Quarter 1: 42')], [
  ...
  it('finds multiple instances across the tree', () => {
    const node = mockEl('svg', [mockAttr('aria-label', 'chart')], [
...
describe('findNodes', () => {
  ...
  it('returns empty array when no match', () => {
    ...
    expect(findNodes(node, (n) =>
      typeof n === 'object' && n !== null && (n as MockNode & { tag?: string }).tag === 'circle',
    ).length).toBe(0);
  });
});
...
describe('chart output structure', () => {
  it('a bar chart renders data-bearing rect elements with aria-labels', () => {
    ...
    const chartOutput = mockEl('svg', [mockRole('img'), mockAttr('aria-label', 'Bar chart')], [
  ...
  it('an accessible SVG has a title child', () => {
    const chartOutput = mockEl('svg', [mockAttr('aria-label', 'Test chart')], [
      mockEl('title', [], ['Test chart']),
      mockEl('g', [], []),
    ]);
