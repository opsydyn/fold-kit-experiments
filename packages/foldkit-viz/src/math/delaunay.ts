...
//
// @ts-nocheck: TypedArray index accesses are within-bounds by algorithm invariants;
// noUncheckedIndexedAccess produces 169 false-positive errors in this numerical port.
...
export function triangulate(points: ReadonlyArray<Point2D>): DelaunayResult {
  ...
  for (let k = 0, xp = 0, yp = 0; k < ids.length; k++) {
    ...
    q = hullNext[nn];
    while (
      orient2d(x, y, coords[2 * nn], coords[2 * nn + 1], coords[2 * q], coords[2 * q + 1]) < 0
    ) {
...
function buildVectors(coords: Float64Array, hull: Uint32Array, n: number): Float64Array {
...
function clipSegment(
  ...
  const flip = c0 < c1;
  if (flip) {
    [sx0, sy0, sx1, sy1, c0, c1] = [sx1, sy1, sx0, sy0, c1, c0];
  }
  while (true) {
    ...
    let x = 0,
      y = 0;
    if (c & 8) {
      x = sx0 + ((sx1 - sx0) * (y1 - sy0)) / (sy1 - sy0);
      y = y1;
    } else if (c & 4) {
      x = sx0 + ((sx1 - sx0) * (y0 - sy0)) / (sy1 - sy0);
      y = y0;
    } else if (c & 2) {
      y = sy0 + ((sy1 - sy0) * (x1 - sx0)) / (sx1 - sx0);
      x = x1;
    } else {
      y = sy0 + ((sy1 - sy0) * (x0 - sx0)) / (sx1 - sx0);
      x = x0;
    }
    if (c0) {
      sx0 = x;
      sy0 = y;
      c0 = regioncode(sx0, sy0, x0, y0, x1, y1);
    } else {
      sx1 = x;
      sy1 = y;
      c1 = regioncode(sx1, sy1, x0, y0, x1, y1);
    }
  }
}
...
function clipFinite(
  ...
  if (P) {
    e0 = e1;
    e1 = edgecode(P[0], P[1], x0, y0, x1, y1);
...
function clipInfinite(
  ...
  if (P.length > 0) {
    for (
      let j = 0, pLen = P.length, c0 = 0, c1 = edgecode(P[pLen - 2], P[pLen - 1], x0, y0, x1, y1);
      j < pLen;
      j += 2
    ) {
      c0 = c1;
      c1 = edgecode(P[j], P[j + 1], x0, y0, x1, y1);
      if (c0 && c1) {
        j = edgeInsert(i, c0, c1, P, j, x0, y0, x1, y1, containsFn);
        pLen = P.length;
      }
    }
  } else if (containsFn(i, (x0 + x1) / 2, (y0 + y1) / 2)) {
...
function computeVoronoiCellsRaw(
  ...
  function step(i: number, x: number, y: number): number {
    ...
    let c = i,
      dc = (x - coords[i * 2]) ** 2 + (y - coords[i * 2 + 1]) ** 2;
    const e0 = inedges[i];
    let e = e0;
    do {
      const t = triangles[e];
...
export function voronoiPath(result: DelaunayResult, bounds: Bounds): string {
  const raw = computeVoronoiCellsRaw(result, bounds);
  const parts: string[] = [];
