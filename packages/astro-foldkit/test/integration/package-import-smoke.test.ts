...
const setupFixture = async (): Promise<SmokeFixture> => {
  ...
  const { stdout } = await execFileAsync('npm', ['pack', '--json'], { cwd: packageDir, env, maxBuffer })
  ...
  const distIndex = await import(pathToFileURL(path.join(pkgRoot, 'dist', 'index.mjs')).href) as DistIndex
  const distDefineApp = await import(pathToFileURL(path.join(pkgRoot, 'dist', 'define-app.mjs')).href) as DistDefineApp
...
describe('packed import smoke', () => {
  it('dist exports resolve correctly (Bun import)', async () => {
    ...
    const integration = (distIndex.default as () => { name: string; hooks: Record<string, unknown> })()
  ...
  it('imports correctly under Bun and Node via consumer script', async () => {
    ...
    for (const [runtime, args] of [['bun', ['-e', importScript]], ['node', ['-e', importScript]]] as const) {
      const { stdout } = await execFileAsync(runtime, args, { cwd: consumerDir, env, maxBuffer })
      const payload = JSON.parse(stdout.trim().split('\n').at(-1) ?? '') as {
        defaultIsFunction: boolean; integrationName: string; hasSetupHook: boolean
      }
    ...
    for (const [runtime, args] of [['bun', ['-e', defineAppScript]], ['node', ['-e', defineAppScript]]] as const) {
      const { stdout } = await execFileAsync(runtime, args, { cwd: consumerDir, env, maxBuffer })
      const payload = JSON.parse(stdout.trim().split('\n').at(-1) ?? '') as {
        defineAppIsFunction: boolean; foldkitFlag: boolean; loadIsFunction: boolean
      }
