...
import { HoveredBar, BlurredBar } from './message';
import type { Message } from './message';
import type { Model } from './model';

const W = 480, H = 260, MT = 20, MR = 20, MB = 40, ML = 44;
...
export const view = (model: Model): Document => {
  ...
  const svgEl: Html = h.svg(
    [
      ...
      h.Width('100%'),
      h.Role('img'),
      h.AriaLabel('Animated bar chart'),
    ],
    [
        ...
        [
          ...
          h.g(
            ...
            yTicks.map((t) => {
              ...
              return h.g([h.Transform(`translate(0,${y})`)], [
                h.line([h.X1('0'), h.Y1('0'), h.X2(String(PW)), h.Y2('0'),
                  h.Stroke('var(--chart-grid, #2d2d2d)'), h.StrokeWidth('1')], []),
                h.text([h.X('-8'), h.Y('0'),
                  h.Style({ 'text-anchor': 'end', 'dominant-baseline': 'middle',
                    'font-size': '0.7rem', fill: 'var(--chart-label, #888)' })],
                  [String(Math.round(t))]),
              ]);
            }),
          ),
          ...
          h.g(
            ...
            bars.map((bar, i) => {
              ...
              const animatedValue = tweenValue(0, bar.value, { progress } as Parameters<typeof tweenValue>[2]);
              ...
              return h.g(
                ...
                [
                  h.rect([
                    ...
                    h.Width(String(bw)), h.Height(String(barH)),
                    h.Fill(bar.color),
                  ...
                  ...(isActive ? [
                    h.text([
                      h.X(String(bx + bw / 2)),
                      h.Y(String(by - 6)),
                      h.Style({ 'text-anchor': 'middle', 'font-size': '0.75rem',
                        'font-weight': '700', fill: bar.color }),
                    ], [String(bar.value)]),
                  ] : []),
                ],
              );
          ...
          h.g(
            ...
            [
              h.line([h.X1('0'), h.Y1('0'), h.X2(String(PW)), h.Y2('0'),
                h.Stroke('var(--chart-axis, #3a3a3a)'), h.StrokeWidth('1')], []),
              ...bars.map((bar) =>
                h.text([
                  h.X(String(Math.round((xScale.position(bar.label) + xScale.bandwidth / 2) * 1000) / 1000)),
                  h.Y('16'),
                  h.Style({ 'text-anchor': 'middle', 'font-size': '0.75rem',
